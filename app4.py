...
def gen_frames():  # generate frame by frame from camera
    img_id=0
    # camera = cv2.VideoCapture("rtsp://admin:L2E66E94@192.168.43.170/cam/realmonitor?channel=1&subtype=00")  # use 0 for web camera
        ...
        if(switch):
            ...
            if not success:
                ...
                if facec is not None:
                    ...
                    if cv2.waitKey(1) == 13 or int(img_id) == 50:  # wait key ==delay
...
# @app.route('/viewstudent/<int:username>/',methods=['POST','GET'])
# def searchstd():
#     student = StudentInfo.query.get_or_404(username) 
#     return render_template('viewstudent.html' , student = student)
#-----------------------------------------------------------------------------------------------------------------------
#Add new Student
...
def loadmodel():
    ...
    #compress the 128 embeddings of each face 
    savez_compressed('Newembeddings.npz',new_trainx,trainy)
    msg1 = "Model has been loaded"
    print(msg1)
...
def Train():
    ...
    print(listdir('newED/'))
    #load the datasets
    trainX,trainY = load_dataset('newED/')
    print(trainX.shape,trainY.shape)
    # print(trainY)
    #compress the data
