    ...
    if file and allowed_file(file.filename):
        ...
        def save_faces(filecname, result_list):
            ...
            for i in range(len(result_list)):
                ...
                cv2.imwrite("static/cropped/img_{}.png".format(i),
                            data[y1:y2, x1:x2])
        ...
        return render_template('index.html', filename=filename, std_list=std_list, st1=unfacecount)
    else:
...
def select_image():
    ...
    cam = cv2.VideoCapture(
        'rtsp://admin:L2E66E94@192.168.0.108/cam/realmonitor?channel=1&subtype=00')
...
def gen_frames():  # generate frame by frame from camera
    ...
    camera = cv2.VideoCapture(
        "rtsp://admin:L2E66E94@192.168.0.108/cam/realmonitor?channel=1&subtype=00")  # use 0 for web camera
        ...
        if (switch):
            ...
            if not success:
                ...
                if facec is not None:
                    ...
                    file_name_path = user_folder+"/" + \
                        username+"."+str(img_id)+".jpg"
    ...
    for x in os.listdir('new/' + username+'/'):
        ...
        if not faces:
            print(
                'There is no face in the provided image, Please select another image.'+x)
        ...
        if coun < 85:
            print(
                "Warning!! Images are less than 85% please regenerte the dataset immediately!")
...
def tasks():
    ...
    if request.method == 'POST':
        ...
        new_register = StudentInfo(username=username, password=password, fname=fname,
                                   lname=lname, dob=dob, gender=gender, email=email, pno=pno, sem=sem)
...
def addnew():
    ...
    def load_dataset(directory):
        ...
        for subdir in listdir(directory):
            ...
            # print(labels)
            # summarize
            print("%d There are %d images in the class %s:" %
                  (i, len(faces), subdir))
            x.extend(faces)
            y.extend(labels)
            i = i+1
    ...
    def extract_embeddings(model, face_pixels):
        # convert the entire data to float32(base)
        face_pixels = face_pixels.astype('float32')
...
def addteacher():
    ...
    if request.method == 'POST':
        ...
        new_register = TeacherInfo(username=username, password=password, fname=fname,
                                   lname=lname, dob=dob, gender=gender, email=email, pno=pno, subject=subject)
